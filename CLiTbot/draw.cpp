...
void chartlet_robot(int row,int col,int dir,int height){//row,col代表机器人的横纵坐标，dir代表朝向 
	switch(dir){
		case 0:{ ifstream up;//以输出的方式打开文件 
				...
				for(int y=0;y<150;y++){
					for(int x=0;x<60;x++){
						Save[y][x]=(pixel){buffer0[i],buffer0[i+1],buffer0[i+2]};
						i=i+3;					
					}
				}
				int m,n;
		...
		case 1:{ifstream down;//以输出的方式打开文件 
				...
				for(int y=0;y<150;y++){
					for(int x=0;x<60;x++){
						Save[y][x]=(pixel){buffer1[i],buffer1[i+1],buffer1[i+2]};
						i=i+3;					
					}
				}
				int m,n;
		...
		case 2:{ifstream le;//以输出的方式打开文件 
				...
				for(int y=0;y<150;y++){
					for(int x=0;x<60;x++){
						Save[y][x]=(pixel){buffer2[i],buffer2[i+1],buffer2[i+2]};
						i=i+3;					
					}
				}
				int m,n;
		...
		case 3:{ifstream ri;//以输出的方式打开文件 
				...
				for(int y=0;y<150;y++){
					for(int x=0;x<60;x++){
						Save[y][x]=(pixel){buffer3[i],buffer3[i+1],buffer3[i+2]};
						i=i+3;					
					}
				}
				int m,n;
...
void string_splicing(string path,int id,string savepath,int time){
  for(int i=0;i<=100;i++){
  	if(path[i]=='%'&&path[i+2]=='d'){
  		...
  		savepath=path.substr(0,i)+to_string(id+time)+".bmp";
		  break;}
		  ...
		  savepath=savepath+to_string(id+time)+".bmp";
		  break;
	  }
    }
  }
...
void auto_save(){
	if(game.save_path.empty()) return;//关闭自动保存的等价表示 
		...
		string auto_path;
		string_splicing(game.save_path,game.auto_save_id,auto_path,time);//逻辑还需细化与微调 
