...
const pixel YELLOW_PIXEL = { 0,255,255 };
const pixel RED_PIXEL = { 0,0,255 };
const pixel BLUE_PIXEL = { 255,47,47 };
const pixel PURPLE_PIXEL = { 163,73,164 };
const pixel GRAY_PIXEL = { 50,50,50 };
int time_ = 0;
			...
			for (int k = 450 + (col - row) * 150, j = 675 - 75 * (col + row) + (i - 1) * 35; j <= 675 - 75 * (col + row) + (i - 1) * 35 + 35; j++) {//写循环 
				for (int y = j, x = k; x <= k + 150; y--, x += 2) {
					if (y == j || j == 675 - 75 * (col + row) + (i - 1) * 35 || j == 675 - 75 * (col + row) + (i - 1) * 35 + 35 || y == j - 75 || y == j - 1 || y == j - 74 || j == 675 - 75 * (col + row) + (i - 1) * 35 + 34 || j == 675 - 75 * (col + row) + (i - 1) * 35 + 1) {
						content[y][x] = BLACK_PIXEL;
						content[y - 1][x - 1] = BLACK_PIXEL;
						content[y - 1][x] = BLACK_PIXEL;
						content[y][x - 1] = BLACK_PIXEL;
					}
					else {
						content[y][x] = RED_PIXEL;
						content[y - 1][x - 1] = RED_PIXEL;
						content[y - 1][x] = RED_PIXEL;
						content[y][x - 1] = RED_PIXEL;
					}
				}
			}
		...
		for (int i = 450 + (col - row) * 150, j = 675 - 75 * (col + row) + (height - 1) * 35; i <= 600 + (col - row) * 150; i += 2, j--) {//写循环 
			for (int y = j, x = i; x < i + 150; y++, x += 2) {
				if (i == 450 - (row - col) * 150 || i == 452 - (row - col) * 150 || i == 598 - (row - col) * 150 || i == 600 - (row - col) * 150 || y == j || y == j - 1 || x == i + 148 || x == i + 146) {
					...
					if (exist_light == -1) {
						content[y][x] = BLUE_PIXEL;
						content[y - 1][x - 1] = BLUE_PIXEL;
						content[y - 1][x] = BLUE_PIXEL;
						content[y][x - 1] = BLUE_PIXEL;
					}
					else {
						content[y][x] = GRAY_PIXEL;
						content[y - 1][x - 1] = GRAY_PIXEL;
						content[y - 1][x] = GRAY_PIXEL;
						content[y][x - 1] = GRAY_PIXEL;
					}
			}
		}
	}
		...
		for (int i = 450 - (row - col) * 150, j = 675 - 75 * (col + row) + (height - 1) * 35; i <= 600 - (row - col) * 150; i += 2, j--) {//写循环 
			for (int y = j, x = i; x < i + 150; y++, x += 2) {
				if (i == 450 - (row - col) * 150 || i == 452 - (row - col) * 150 || i == 598 - (row - col) * 150 || i == 600 - (row - col) * 150 || y == j || y == j - 1 || x == i + 148 || x == i + 146) {
					...
					content[y][x] = YELLOW_PIXEL;
					content[y - 1][x - 1] = YELLOW_PIXEL;
					content[y - 1][x] = YELLOW_PIXEL;
					content[y][x - 1] = YELLOW_PIXEL;
				}
			}
		}
	}
